...
def write_grayscale(filename, pixels):
    """Creates and writes a grayscale BNP file.
    ...
    with open(filename, mode='wb') as bmp:
        ...
        size_bookmark = bmp.tell()      # The next four bytes hold the file size as a 32-bit
        ...
        bmp.write(b'\x01\x00')          # Number of image plane
        ...
        for row in reversed(pixels):
            row_data = bytes(row)
            bmp.write(row_data)
            padding = b'\x00' * ((4 - (len(row) % 4)) % 4)  # Pad rows to multiple of four bytes
            bmp.write(padding)
...
def _int32_to_bytes(i):
    """Converts an integer to four bytes in little-endian format."""
    # &: Bitwise-and
    # >>: Right-shift
    return bytes((i & 0xff,
                  i >> 8 & 0xff,
